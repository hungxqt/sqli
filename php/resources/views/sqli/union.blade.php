...
<body>
    <div class="container">
        ...
        <form method="GET" action="{{ route('sqli.union') }}">
        ...
        @elseif($searchTerm !== null)
            <div class="result-box {{ count($users) > 0 ? 'success' : '' }}">
                    ...
                    <table>
                        ...
                        <tbody>
                            @foreach($users as $user)
                                <tr><td>{{ $user->id ?? 'N/A' }}</td><td>{{ $user->username ?? 'N/A' }}</td><td>{{ $user->password ?? 'N/A' }}</td></tr>
                            @endforeach
                        </tbody>
                    </table>
                @else
